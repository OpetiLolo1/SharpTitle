...
class OpeningWindow:  # first window
    ...
    def __init__(self, parent):
        ...
        self.opening_frame = Frame(parent, bg=background_color,
                                   padx=50, pady=130)
        self.opening_frame.grid()

        self.title_label = Label(self.opening_frame, text='MRGS Booking'
        ...
        self.exit_button = Button(
            ...
            bg='red',
            font=('Helvetica', '15'),
            command=self.opening_frame.destroy,
            )
...
class RegisterFrame(Frame):  # this frame sits on the Main App window
    ...
    def __init__(self, parent, controller):
        ...
        self.user_label = Label(self.border, text='Student ID',
                                fg='white', font=('Helvetica Bold',
                                18), bg='red3')
        ...
        self.password_label = Label(self.border, text='Password',
                                    fg='white', font=('Helvetica Bold',
                                    18), bg='red3')
        self.password_label.place(x=200, y=125)

        self.password_entry = Entry(self.border, width=30, show='*',
                                    bd=5)
        ...
        def verify():
            ...
            try:
                with open('UsersIDandPassword.txt', 'r') as f:  # gets the usernames and passwords from the file UsersIDandPassword.txt
                    info = f.readlines()
                    i = 0
                    for e in info:
                        (self.user_name, self.user_password) = \
                            e.split(',')  # used to split the username from the password with a comma in the file
                    ...
                    if i == 0:
                        messagebox.showinfo('Error',
                                'Please provide correct student ID and password!!'
                                )
            except:
        ...
        def register():
            ...
            reg_name_label = Label(register_window, text='Student ID:',
            ...
            reg_password_label = Label(register_window, text='Password:'
                    , fg='white', font=('Helvetica', 15), bg='red3')
            reg_password_label.place(x=10, y=60)
            reg_password_entry = Entry(register_window, width=30,
                    show='*', bd=5)
            reg_password_entry.place(x=220, y=60)

            confirm_password_label = Label(register_window,
                    text='Confirm Password:', fg='white',
                    font=('Helvetica', 15), bg='red3')
            confirm_password_label.place(x=10, y=110)
            confirm_password_entry = Entry(register_window, width=30,
                    show='*', bd=5)
            confirm_password_entry.place(x=220, y=110)

            def check():

                if reg_name_entry.get() != '' \
                    and reg_password_entry.get() != '' \
                    and confirm_password_entry.get() != '':  # all entry boxes must be completed and filled
                    if reg_password_entry.get() \
                        == confirm_password_entry.get():
                        with open('UsersIDandPassword.txt', 'a') as f:
                            f.write(reg_name_entry.get() + ','
                        ...
                        messagebox.showinfo('Error',
                                "Your password didn't get match!!")
                else:
                    messagebox.showinfo('Error',
                            'Please fill the complete field!!')
...
class Second(Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.border = LabelFrame(self, fg='white', bg='red3', bd=10,
                                 font=('Helvetica', '25', 'bold'))
        self.border.pack(fill='both', expand='yes', padx=0, pady=100)

        self.title_label = Label(self,
                                 text='Please type which periods you need a chromebook for'
                                 , fg='white', bg='red3',
                                 font=('Helvetcia Bold', 20))
        self.title_label.place(x=40, y=150)

        self.title_label = Label(self, text='E.g. 1,2,3,4,5', fg='white'
                                 , bg='red3', font=('Helvetica Bold',
                                 20))
        ...
        self.next_button = Button(
            ...
            bg='green',
            font=('Arial', 15),
            command=lambda : controller.show_frame(Third),
            )
...
class Third(Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.app_label = Label(self,
                               text='Thank you for booking \n Please head to B6 to pick up your chromebook.'
                               , bg='red3', fg='white',
                               font=('Arial Bold', 20))
